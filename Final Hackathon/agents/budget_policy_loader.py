...
#                 description="Extract budget data from uploaded Excel, PDF, or DOCX file",
#                 func=self.extract_budget_from_file
#             ),
#             Tool(
#                 name="structure_budget_rules",
#                 description="Structure extracted budget data into JSON format with departments/vendors/categories",
#                 func=self.structure_budget_rules
#             ),
#             Tool(
#                 name="create_rag_vectorstore",
#                 description="Create RAG vector store for budget policy retrieval",
...
#             template="""
#             You are a Budget Policy Loader Agent with RAG capabilities.
            
#             Your mission: Extract and structure budget rules from company documents.
            
#             Tasks:
#             1. Extract budget thresholds, limits, and constraints from uploaded files
#             2. Identify department/vendor/category budget allocations  
#             3. Structure data into standardized JSON format
#             4. Create searchable vector store for RAG retrieval
...
#             Thought: I need to process the budget document and extract structured budget rules.
#             {agent_scratchpad}
#             """
#         )
...
#     def structure_budget_rules(self, extracted_content: str) -> str:
#         """Structure budget data into JSON format with departments/vendors/categories"""
#         try:
#             # Use LLM to structure the extracted content
#             structure_prompt = f"""
#             Analyze this budget document and extract structured budget information.
...
#                         "total_budget": 30000,
#                         "categories": {{
#                             "Advertising": {{"limit": 15000, "period": "monthly"}},
...
#             {extracted_content if hasattr(self, 'extracted_content') else extracted_content}
            
#             Extract all budget limits, department allocations, vendor restrictions, and spending categories.
...
#             try:
#                 self.budget_data = json.loads(structured_data)
#                 # Save structured budget data
#                 os.makedirs(config.UPLOAD_PATH, exist_ok=True)
#                 with open(f"{config.UPLOAD_PATH}/structured_budget.json", 'w') as f:
#                     json.dump(self.budget_data, f, indent=2)
                    
#                 return f"Budget rules structured successfully: {structured_data}"
#             except json.JSONDecodeError:
#                 # If not valid JSON, return the LLM response anyway
#                 return f"Budget analysis completed: {structured_data}"
                
#         except Exception as e:
#             return f"Error structuring budget rules: {str(e)}"
    
#     def create_rag_vectorstore(self, content: str) -> str:
#         """Create FAISS vector store for RAG retrieval of budget policies"""
#         try:
#             # Use extracted content if available
#             document_content = getattr(self, 'extracted_content', content)
...
#             return f"RAG vector store created successfully with {len(chunks)} chunks. Ready for budget policy retrieval."
